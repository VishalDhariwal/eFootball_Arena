...
import { X, AlertCircle, IndianRupee, Phone, CheckCircle, Loader2 } from "lucide-react";
import { Button } from "@/components/ui/button";

interface RefundRequestModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: { upiId?: string; phone?: string; reason?: string }) => Promise<void>;
...
export const RefundRequestModal = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit({ upiId: cleanUpi, phone: cleanPhone, reason: reason.trim() });
      // Reset form on success is handled by unmounting or the parent
      setUpiId("");
      setPhone("");
      setReason("");
    } catch (err: any) {
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-muted/20">
                <div>
                  <h3 className="font-semibold text-white">Request Refund</h3>
                  <p className="text-xs text-muted-foreground mt-0.5">{tournamentName}</p>
                </div>
                <button
                  onClick={onClose}
              ...
              <div className="p-5 overflow-y-auto">
                <p className="text-sm text-muted-foreground mb-4">
                  Please provide the payment details where you'd like to receive your refund.
                  <strong className="text-white"> Either UPI ID or Phone Number is required.</strong>
                </p>
                ...
                <form id="refund-form" onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="pt-2">
                    <label className="block text-xs font-medium text-muted-foreground mb-1.5">
                      Reason for refund <span className="text-muted-foreground/60">(optional)</span>
                    </label>
                    <textarea
                      value={reason}
                      ...
                      rows={3}
                      className="w-full bg-background border border-border rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary transition-colors resize-none"
                    />
                  </div>
              ...
              <div className="px-5 py-4 border-t border-border bg-muted/10 flex justify-end gap-3">
                ...
                <Button 
                  type="submit" 
                  form="refund-form" 
                  className="bg-yellow-500 hover:bg-yellow-400 text-black shadow-none font-semibold"
